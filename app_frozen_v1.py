# Full updated Flask app with messaging, CSRF protection, Flask-Migrate, safer notification API,
# allowed upload checks, and messaging endpoints. Update your templates to match the new routes
# (messages.html, message_thread.html, message_compose snippet, and a messages/unread badge).
#
# After saving this file:
# 1) pip install -r requirements.txt (ensure flask, flask_sqlalchemy, flask_migrate, flask_login, flask_wtf, email-validator)
# 2) export DATABASE_URL=sqlite:///job_platform.db
#    export SECRET_KEY="your-secret"
#    optionally export DEFAULT_ADMIN_EMAIL and DEFAULT_ADMIN_PASSWORD to create an admin at startup
# 3) flask db init
...
app = Flask(__name__)
app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY') or 'change-this-secret-in-production'
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get(
    'DATABASE_URL'
) or 'sqlite:///' + os.path.join(BASE_DIR, 'job_platform.db')
...
class User(UserMixin, db.Model):
    ...
    national_id = db.Column(db.String(50))  # Removed unique constraint due to NULL issues
...
class Notification(db.Model):
    ...
    def to_dict(self):
        ...
        created_iso = self.created_at.replace(microsecond=0).isoformat() + 'Z'
        return {
            'id': self.id,
...
def create_notification(user_id, message, type='info', target_url=None, commit=False):
    """Create a notification (does not commit by default to allow transactions)"""
...
def ensure_default_admin():
    """Create an admin only if env vars provided (safer than hardcoding)"""
    ...
    if request.method == 'POST':
        ...
        if cert and cert.filename:
            if allowed_file(cert.filename):
                ...
                flash('Unsupported file type for certificate. Allowed: PDF, PNG, JPG, JPEG', 'warning')
                return redirect(url_for('register'))
    ...
    if request.method == 'POST':
        ...
        if user and user.check_password(password):
            if user.burned:
                ...
                return redirect(url_for('login'))
            if user.suspended:
                flash('Your account is suspended. Contact admin for details.', 'warning')
                return redirect(url_for('login'))
...
def profile(user_id=None):
    ...
    # Applications - only show if viewing own profile
    applications = []
    if is_own_profile:
...
def messages_send_api():
    """Send message API (for AJAX requests)"""
